...
class FakeRemoteAdapter implements RemoteAdapter {
  ...
  Future<PullResult> pull(PullRequest request) async {
    ...
    source.sort((a, b) {
      ...
      return a[request.primaryKey].toString().compareTo(
        b[request.primaryKey].toString(),
      );
    });
    ...
    final filtered = source.where((row) {
      ...
      return row[request.primaryKey].toString().compareTo(
            request.cursor!.primaryKey.toString(),
          ) >
          0;
    }).toList();
  ...
  Future<void> softDelete({
    ...
    if (idempotencyKey != null &&
        failOnceOpIds.contains(idempotencyKey) &&
        !failedOpIds.contains(idempotencyKey)) {
...
void main() {
  group('phase 1 correctness', () {
    test('init ensures columns for registered tables', () async {
      ...
      await engine.init();
      expect(
        local.ensuredTables.single,
        contains('notes:updated_at:deleted_at'),
      );
    });
    ...
    test(
      'composite cursor paginates same timestamp records without skipping',
      () async {
        final local = InMemoryLocalStore();
        final remote = FakeRemoteAdapter();
        remote.remoteTables['notes'] = [
    ...
    test('serverWins overwrites dirty local', () async {
      ...
      final engine = SyncEngine(localStore: local, remoteAdapter: remote)
        ..registerTable(notesTable(strategy: SyncStrategy.serverWins));
    ...
    test('localWins keeps dirty local', () async {
      ...
      final engine = SyncEngine(localStore: local, remoteAdapter: remote)
        ..registerTable(notesTable(strategy: SyncStrategy.localWins));
    ...
    test('lastWriteWins chooses remote when remote newer', () async {
      ...
      final engine = SyncEngine(localStore: local, remoteAdapter: remote)
        ..registerTable(notesTable(strategy: SyncStrategy.lastWriteWins));
    ...
    test('custom resolver is used for dirty conflicts', () async {
      ...
      final config = TableConfig(
        name: 'notes',
        primaryKey: 'uuid',
        columns: ['uuid', 'title', 'updated_at'],
        strategy: SyncStrategy.custom,
        customResolver: mergeResolver,
      );

      final engine = SyncEngine(localStore: local, remoteAdapter: remote)
        ..registerTable(config);

      await engine.syncTable(config);
